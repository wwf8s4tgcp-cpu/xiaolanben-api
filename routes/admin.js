...
const postsCrudConfig = {
  ...
  beforeCreate: async (data, req) => {
    ...
    // 检查用户是否存在
    const { rows: userResult } = await execute('SELECT id FROM users WHERE id = ?', [String(user_id)])
    ...
    if (tags && tags.length > 0) {
      for (const tag of tags) {
        ...
        if (typeof tag === 'string') {
          tagName = tag
          // 查找现有标签
          const { rows: existingTag } = await execute(
          ...
          if (existingTag.length > 0) {
            ...
            // 创建新标签
            const { rows: tagResult } = await execute(
              'INSERT INTO tags (name) VALUES (?) RETURNING id',
              [tagName]
            )
            tagId = String(tagResult[0].id)
          }
        } else {
          // 处理对象格式的标签（向后兼容）
          tagId = tag.id
  ...
  afterUpdate: async (postId, data, req) => {
    ...
    if (images !== undefined || image_urls !== undefined) {
      // 删除原有图片
      await execute('DELETE FROM post_images WHERE post_id = ?', [String(postId)])
    ...
    if (hasVideoUpdate) {
      // 获取原有视频记录用于清理文件
      const { rows: oldVideoRows } = await execute('SELECT video_url, cover_url FROM post_videos WHERE post_id = ?', [String(postId)])

      // 删除原有视频记录
      await execute('DELETE FROM post_videos WHERE post_id = ?', [String(postId)])
    ...
    if (tags !== undefined) {
      // 获取原有标签，用于更新使用次数
      const { rows: oldTags } = await execute(
  ...
  beforeDelete: async (id) => {
    ...
    for (const tag of tagResult) {
      await execute('UPDATE tags SET use_count = use_count - 1 WHERE id = ?', [String(tag.tag_id)])
    }
    // 返回验证结果
    return { isValid: true }
  },
  ...
  customQueries: {
    getOne: async (req) => {
      ...
      // 获取笔记基本信息
      const { rows: postResult } = await execute(`
        SELECT p.id, p.user_id, p.title, p.content, p.type, p.category_id, c.name as category,
               ...
               u.nickname, COALESCE(u.user_id, 'user' || LPAD(u.id::text, 3, '0')) as user_display_id
        FROM posts p
      ...
      if (post.type === 2) {
        // 视频笔记：获取视频信息
        const { rows: videos } = await execute('SELECT video_url, cover_url FROM post_videos WHERE post_id = ?', [String(postId)])
        if (videos.length > 0) {
        ...
        // 图文笔记：获取图片信息
        const { rows: images } = await execute('SELECT image_url FROM post_images WHERE post_id = ?', [String(postId)])
        post.images = images.map(img => img.image_url)
      }

      // 获取笔记标签
      const { rows: tags } = await execute(`
        SELECT t.id, t.name 
        FROM tags t 
    ...
    getList: async (req) => {
      ...
      const countQuery = `
        ...
        ${whereClause}
      `
      const { rows: countResult } = await execute(countQuery, params)
      ...
      const dataQuery = `
        ...
        LIMIT ? OFFSET ?
      `
      const { rows: posts } = await execute(dataQuery, [...params, String(limit), String(offset)])
      ...
      if (posts.length > 0) {
        ...
        // 批量获取图片
        const { rows: images } = await execute('SELECT post_id, image_url FROM post_images WHERE post_id IN (?)', [postIds])
        const imageMap = {}
        images.forEach(img => {
...
router.put('/posts-audit/:id/approve', adminAuth, async (req, res) => {
  try {
    ...
    // 检查笔记是否存在
    const { rows: postResult } = await execute('SELECT id FROM posts WHERE id = ?', [String(postId)])
    if (postResult.length === 0) {
    ...
    // 更新笔记状态为已发布
    await execute('UPDATE posts SET status = 0 WHERE id = ?', [String(postId)])

    // 检查笔记内容是否包含@用户，如果有，发送艾特通知
    const { rows: postContentResult } = await execute('SELECT user_id, content FROM posts WHERE id = ?', [String(postId)])
    if (postContentResult.length > 0) {
      ...
      if (content && hasMentions(content)) {
        ...
        for (const mentionedUser of mentionedUsers) {
          try {
            // 根据小蓝本号查找用户的自增ID
            const { rows: userRows } = await execute('SELECT id FROM users WHERE user_id = ?', [mentionedUser.userId])
...
router.put('/posts-audit/:id/reject', adminAuth, async (req, res) => {
  try {
    ...
    // 未过审时设置为未过审状态
    await execute('UPDATE posts SET status = 3 WHERE id = ?', [String(postId)])
...
const commentsCrudConfig = {
  ...
  beforeCreate: async (data) => {
    ...
    // 检查笔记是否存在
    const { rows: postResult } = await execute('SELECT id FROM posts WHERE id = ?', [String(post_id)])
    if (postResult.length === 0) {
    ...
    // 如果是回复评论，检查父评论是否存在
    if (parent_id) {
      const { rows: parentResult } = await execute('SELECT id FROM comments WHERE id = ?', [String(parent_id)])
    ...
    getList: async (req) => {
      ...
      const dataQuery = `
        SELECT c.id, c.content, c.parent_id, c.like_count, c.created_at,
               ...
               COALESCE(u.user_id, 'user' || LPAD(u.id::text, 3, '0')) as user_display_id,
        ...
        LIMIT ? OFFSET ?
      `
      const { rows: comments } = await execute(dataQuery, [...params, String(limit), String(offset)])
  ...
  customQueries: {
    getList: async (req) => {
      ...
      const dataQuery = `
        ...
        LIMIT ? OFFSET ?
      `
      const { rows: likes } = await execute(dataQuery, [...params, String(limit), String(offset)])
  ...
  customQueries: {
    getList: async (req) => {
      ...
      const dataQuery = `
        ...
        LIMIT ? OFFSET ?
      `
      const { rows: collections } = await execute(dataQuery, [...params, String(limit), String(offset)])
...
const followsCrudConfig = {
  ...
  beforeUpdate: async (data, id) => {
    if (data.following_id) {
      // 获取当前记录的关注者ID
      const { rows: current } = await execute('SELECT follower_id FROM follows WHERE id = ?', [String(id)])
      if (current.length === 0) {
        return {
          isValid: false,
          message: '关注记录不存在',
  ...
  customQueries: {
    getList: async (req) => {
      ...
      if (req.query.follower_display_id) {
        // 根据关注者小蓝本号查找用户ID
        const userQuery = 'SELECT id FROM users WHERE COALESCE(user_id, \'user\' || LPAD(id::text, 3, \'0\')) = ?'
        const { rows: userResult } = await execute(userQuery, [req.query.follower_display_id])
        if (userResult.length > 0) {
      ...
      if (req.query.following_display_id) {
        // 根据被关注者小蓝本号查找用户ID
        const userQuery = 'SELECT id FROM users WHERE COALESCE(user_id, \'user\' || LPAD(id::text, 3, \'0\')) = ?'
        const { rows: userResult } = await execute(userQuery, [req.query.following_display_id])
        if (userResult.length > 0) {
      ...
      const dataQuery = `
        SELECT f.id, f.follower_id, f.following_id, f.created_at,
               ...
               COALESCE(u1.user_id, 'user' || LPAD(u1.id::text, 3, '0')) as follower_display_id,
               u2.nickname as following_nickname, 
               COALESCE(u2.user_id, 'user' || LPAD(u2.id::text, 3, '0')) as following_display_id
        FROM follows f
        ...
        LIMIT ? OFFSET ?
      `
      const { rows: follows } = await execute(dataQuery, [...params, String(limit), String(offset)])
    ...
    getList: async (req) => {
      ...
      const dataQuery = `
        SELECT n.id, n.user_id, n.sender_id, n.type, n.title, n.target_id, n.comment_id, n.is_read, n.created_at,
               ...
               COALESCE(u1.user_id, 'user' || LPAD(u1.id::text, 3, '0')) as user_display_id,
               u2.nickname as sender_nickname, 
               COALESCE(u2.user_id, 'user' || LPAD(u2.id::text, 3, '0')) as sender_display_id
        ...
        LIMIT ? OFFSET ?
      `
      const { rows: notifications } = await execute(dataQuery, [...params, String(limit), String(offset)])
  ...
  customQueries: {
    getList: async (req) => {
      ...
      const dataQuery = `
        ...
        LIMIT ? OFFSET ?
      `
      const { rows: sessions } = await execute(dataQuery, [...params, String(limit), String(offset)])
  ...
  customQueries: {
    getList: async (req) => {
      ...
      const dataQuery = `
        ...
        LIMIT ? OFFSET ?
      `
      const { rows: users } = await execute(dataQuery, [...params, String(limit), String(offset)])
    ...
    getOne: async (req) => {
      ...
      const { rows: userResult } = await execute(`
        SELECT u.*,
               COALESCE(ub.status, -1) as ban_status,
               ub.reason as ban_reason,
               ub.end_time as ban_end_time,
...
async function revokeExistingBans(userId, operatorId) {
  try {
    // 检查是否已经有未解除的封禁记录
    const { rows: existingBan } = await execute(
...
router.post('/users/:id/ban', adminAuth, async (req, res) => {
  try {
    ...
    // 检查用户是否存在
    const { rows: userResult } = await execute('SELECT id FROM users WHERE id = ?', [String(userId)])
    if (userResult.length === 0) {
      return res.status(HTTP_STATUS.NOT_FOUND).json({
        code: RESPONSE_CODES.NOT_FOUND,
        message: '用户不存在'
...
async function unbanUser(userId, operatorId) {
  try {
    // 查找用户的活跃封禁记录
    const { rows: banResult } = await execute(
      'SELECT id FROM user_ban WHERE user_id = ? AND status IN (0, 3)',
      [String(userId)]
    )
...
router.get('/monitor/activities', adminAuth, async (req, res) => {
  try {
    ...
    // 获取最近10个新注册用户
    const { rows: newUsers } = await execute(
    ...
    // 获取最近10篇发布的笔记
    const { rows: newPosts } = await execute(
    ...
    // 获取最近10条评论
    const { rows: newComments } = await execute(
  ...
  customQueries: {
    getList: async (req) => {
      ...
      const [dataResult, countResult] = await Promise.all([
        execute(dataQuery, queryParams),
        execute(countQuery, queryParams.slice(0, -2))
      ])
...
router.put('/audit/:id/approve', adminAuth, async (req, res) => {
  try {
    ...
    // 获取认证记录信息
    const { rows: verificationResult } = await execute('SELECT user_id, type FROM user_verification WHERE id = ?', [id])
    ...
    try {
      // 更新认证状态为通过 (1)
      await execute('UPDATE user_verification SET status = 1 WHERE id = ?', [id])

      // 根据认证类型更新用户的verified字段
      // type: 1-官方认证, 2-个人认证
      const verifiedValue = type === 1 ? 1 : (type === 2 ? 2 : 0)
      await execute('UPDATE users SET verified = ? WHERE id = ?', [verifiedValue, user_id])
...
router.put('/audit/:id/reject', adminAuth, async (req, res) => {
  try {
    ...
    try {
      // 更新认证状态为拒绝 (2)
      await execute('UPDATE user_verification SET status = 2 WHERE id = ?', [id])

      // 拒绝认证申请时，将用户的verified字段设置为0（未认证）
      await execute('UPDATE users SET verified = 0 WHERE id = ?', [user_id])
...
const categoriesCrudConfig = {
  ...
  beforeCreate: async (data) => {
    ...
    // 检查分类名称是否已存在
    const { rows: existingName } = await execute(
    ...
    // 检查分类英文标题是否已存在
    const { rows: existingTitle } = await execute(
  ...
  beforeDelete: async (id) => {
    // 检查是否有笔记使用此分类
    const { rows: posts } = await execute(
  ...
  customQueries: {
    create: async (req) => {
      ...
      // 创建分类
      const { rows: result } = await execute(
        'INSERT INTO categories (name, category_title) VALUES (?, ?) RETURNING id',
    ...
    getList: async (req) => {
      ...
      // 获取总数
      const { rows: countResult } = await execute(`
