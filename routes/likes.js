...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    // 检查是否已经点赞
    const { rows: existingLikeRows } = await pool.query(
    ...
    if (existingLikeRows.length > 0) {
      ...
      if (target_type == 1) {
        // 笔记
        await pool.query('UPDATE "posts" SET "like_count" = "like_count" - 1 WHERE "id" = $1', [String(target_id)]);

        // 更新笔记作者的获赞数
        await pool.query(
          'UPDATE "users" SET "like_count" = "like_count" - 1 WHERE "id" = (SELECT "user_id" FROM "posts" WHERE "id" = $1)',
          [String(target_id)]
        );
      } else if (target_type == 2) {
        // 评论
        await pool.query('UPDATE "comments" SET "like_count" = "like_count" - 1 WHERE "id" = $1', [String(target_id)]);
      }
      ...
      if (target_type == 1) {
        ...
        // 获取笔记作者ID，用于创建通知
        const { rows: postResult } = await pool.query('SELECT "user_id" FROM "posts" WHERE "id" = $1', [String(target_id)]);
        ...
        const { rows: commentResult } = await pool.query('SELECT "user_id", "post_id" FROM "comments" WHERE "id" = $1', [String(target_id)]);
